...
cf <- causal_forest(X = covariates, 
                    ...
                    min.node.size = 5,
                    honesty = TRUE, # DOPO DOVREMO CAMBIARE QUESTO <- <- <- <- <- <- <- <-
...
colnames(covariates)[ranked.vars[1:3]]
```
...
In order to account for this, we decide to include `target.sample = "overlap"` when calculating the ATE.
...
```{r}
blp <- best_linear_projection(cf, covariates)
print(blp)
```
...
ggplot(cate_data, aes(x = cate)) +
  ...
  theme_minimal()
```
Now we implement a trial and error strategy to find potential drivers for heterogeneity
...
scatter_data <- data.frame(
  driver = df$childcare_availability,
  cate = cate
)
...
```{r}
#| label: Heterogeneity Driver: unemployment_rate

scatter_data <- data.frame(
  driver = df$unemployment_rate,
  cate = cate
)
...
```{r}
#| label: Heterogeneity Driver: #INSERISCI NOME VARIABILE

scatter_data <- data.frame(
  driver = df$unemployment_rate, #BASTA CAMBIARE QUESTA RIGA DI CODICE ED IL RESTO VA IN AUTOMATICO.
  cate = cate
)
...
rate.educ.rate <- rank_average_treatment_effect(
  cf,
  covariates$education_rate, # Multiply by -1 to order by decreasing index
