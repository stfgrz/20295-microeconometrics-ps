...
/* (g) Use the Stata package "twowayfeweights" (or its R version, "TwoWayFEWeights"), based on De Chaisemartin and d'Haultfoeuille (2020), to estimate the weights attached to the regressions you estimated before. Can you explain why the sign of the estimated effect has changed between the regression on Y and the one on Y4? */
	...
	De Chaisemartin and d'Haultfoeuille (2020) show that in a fixed effect design with staggered implementation and heterogeneous treatment effects the estimated coefficient β_fe for the treatment effect could be decomposed as the expectation of a weighted sum of the Δg,t terms, namely the average treatment effect in group g and period t coming from all pairwise difference in differences. The issue is that weights can assume negative values, yielding a downward bias and possibly leading to estimated negative treatment effect even if the average treatment effects on the treated are all positive. Negative weights arise as a result of relying on Difference in Differences where in some cases the control group is already treated due to staggered treatment adoption. 
	
	Weights are computed using the residual of a regression of the treatment dummy on fixed and time effect, that is basically obtained through demeaning. Hence, demeaning on control groups that were already treated is more likely to yield negative weights. 
...
Greater increases in E[Δ2,3] − E[Δ2,2] bias downward the β_fe, which explains why bigger increases in the effect at year 3 for group 2 leads to higher biases. 
...
restore
	...
	/* Goodman-Bacon (2021) provides a decomposition of the twoway Difference in Differences estimator (TWFEDD) in case of staggered treatment as the weighted average of all possible 2x2 two group two periods DiD estimators. These differ by the adopted control group: as a group might change treatment status over time, every DiD can either have as control group an always treated group, a never treated group, or timing groups, namely groups whose treatment stated at different times is used as other's controls groups. The weights given to the 2x2 DiD depend on the timing group sizes and the variance of the treatment dummy in each pair, which tends to be higher in observations in the middle of the panel. With time-invariant treatment effect the estimator returns an average of cross-group treatment effects where weights depend on the variances and are all positive. In case of heterogeneous treatment effect, negative weights are a result of adopting already-treated units as controls, where the treatment effect might change over time but is subtracted in the pairwise DiD. The authors point out that this does not violate the parallel assumption per se but underscore the need for caution when using the TWFEDD estimator. They suggest the adoption of different strategies, namely event studies, stacked
DD or reweighting estimators. The assumption is pairwise common trends in untreated potential outcomes, which result in an average of the difference in counterfactual trends between pairs of timing groups and different time periods adopting the weights stemming from the decomposition theorem, the so called `variance-weighted common trends'(VWCT). This enters the decomposition of the probability limit of the TWFEDD Estimator, along with the variance-weighted average treatment effect on the treated (VWATT) and the the weighted sum of the change in treatment effects within each timing group before and after a later treatment time (deltaATT). As the VWCT is assumed to be equal to zero, when time-varying
treatment effects arise, deltaATT is non-null and biases the estimate while yielding negative weights. 

Plotting the Bacon decomposition for our sample we notice that the assigned weights are all positive as every observation is on the right hand side of the y axis. The estimated coefficient for IMP_UNILATERAL is negative and thus is likely due to a slight predominance of negative estimated average treatment effects in pairwise difference in differences. The large majority of weights is given to never treated units used as control groups, while "timing groups" and "always treated" units are given weights close to 0. Because the latter group is usually the most likely source of negative weights but here has negligible importance, this seems to rule out issues with negative weighting. 
*/
